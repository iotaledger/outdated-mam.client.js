...
const MaskedAuthenticatedMessage = function (options) {
    ...
    /*
    //const normalizedHash = bundle.normalizedBundle(hash).slice(0,27);
    //const signature = Crypto.signing.signatureFragment(normalizedHash, Crypto.converter.trytes(key.key.key));
    const addr = key.key.hash.toString();
    const normalizedHash = bundle.normalizedBundle(hash).slice(0, 27);
    const signingKey = Crypto.converter.trytes(Crypto.signing.address(Crypto.signing.digest(normalizedHash, signature)))
    */
...
const messageID = function (channelKey) {
    const curl = new Crypto.curl();
    const channelTrits = Crypto.converter.trits(channelKey);
...
const parse = function(result, options) {
    ...
    const key = Encryption.key(options.seed, 1, 1, cipher.length*3);
