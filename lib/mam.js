...
const MaskedAuthenticatedMessage = function (options) {
    ...
    const encryptionkey = Encryption.key(options.channelKey, options.encryptionKeyIndex, options.channelKeyIndex, messageTrytes.length*3);
    ...
    const signatureMessageLength = 1;
    ...
    const trytes = Crypto.transactionTrytes(bundle.bundle[0]);
