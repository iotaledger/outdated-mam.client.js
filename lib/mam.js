...
const create = function (options) {
    ...
    const salt = Crypto.converter.trytes(new Int32Array(81).map(() => Math.round(Math.random()* 2 - 1)));
    ...
    trytes.nextKey = Crypto.converter.trytes(channelKey(Crypto.converter.trits(options.channelKey), Crypto.converter.trits(salt)));
    return trytes;
}
...
const parse = function(options) {
    ...
    const nextKey = channelKey(Crypto.converter.trits(options.key), Crypto.converter.trits(salt));
